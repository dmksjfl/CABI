...
class OnlineCallBackFunction(CallBackFunction):
    ...
    def __call__(self, policy, epoch, mean, std, normalize=False) -> dict:
        assert self.is_initialized, "`initialize` should be called before callback."
        policy = deepcopy(policy).cpu()
        eval_res = OrderedDict()
        if not ray.is_initialized():
            ray.init(ignore_reinit_error=True)
        eval_res.update(test_on_real_env(self.task, epoch, d4rl_score, policy, self.env, mean, std, normalize, number_of_runs=self.number_of_runs))
        return eval_res
