...
from offlinerl.utils.data import to_torch, sample, Experience
...
class AlgoTrainer(BaseAlgo):
    ...
    def train(self, train_buffer, val_buffer, callback_fn, weights=None):
        if weights is None:
            ...
            proportional = Experience(int(4e6), alpha=1.0)
            for i in range(weights.shape[0]):
                proportional.add(weights[i])

        for epoch in range(self.args['max_epoch']):
            for i in range(self.args['steps_per_epoch']):
                real_batch_size = int(self.batch_size * self.args['real_data_ratio'])
                model_batch_size = int(self.batch_size - real_batch_size)
                batch_data = train_buffer.sample(real_batch_size, proportional)
                model_data = val_buffer.sample(model_batch_size)
                batch_data.cat_(model_data)
                ...
                vae_loss = self.args['kl_coef'] * kl_loss + recon_loss
            ...
            std = 1.
            normalize=False
            res = callback_fn(self.get_policy(), epoch, mean, std, normalize)
